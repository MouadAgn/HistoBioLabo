...
const documents = [
  ...
  { title: "Photos microscopiques jointes au CR", category: "Diagnostic", description: "Captures haute résolution des coupes histologiques analysées au microscope.", images: ["/documents/2 Photos microscopiques jointes au CR/1B.png", "/documents/2 Photos microscopiques jointes au CR/2B.png"] },
  { title: "Documents envoyés directement", category: "Communication", description: "Accès rapide aux rapports et documents administratifs transmis de manière sécurisée.", images: ["/documents/3 Documents envoyé directement/photo_2026-02-06_10-25-56 (2).jpg", "/documents/3 Documents envoyé directement/photo_2026-02-06_10-25-56.jpg"] },
  { title: "Votre compte HBL", category: "Espace Client", description: "Interface personnalisée permettant la gestion de vos analyses et comptes-rendus.", images: ["/documents/4 Votre compte HBL/Screenshot 2026-02-06 113128.png"] }
]
...
export function ServicesSection() {
  ...
  // Configuration plus nerveuse (stiffness augmentée) pour plus de rapidité
  ...
  return (
    <div className="flex flex-col bg-white">
      {/* SECTION 1 : SERVICES - Hauteur réduite pour défilement plus rapide */}
              ...
              <AnimatePresence mode="wait">
                <motion.div key={activeSrv} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.3 }} className="flex flex-col lg:flex-row gap-6 md:gap-24 items-center w-full">
                  <div className="w-full lg:w-3/5">
                    <div
                      className={`relative w-full rounded-3xl overflow-hidden shadow-2xl border border-slate-100 bg-white ${
                        services[activeSrv].images.length > 1
                          ? "h-[260px] sm:h-[340px] md:h-[520px]"
                      ...
                      {services[activeSrv].images.length > 1 ? (
                        <div className="grid h-full w-full grid-cols-1 gap-4 p-4 sm:grid-cols-2">
                          {services[activeSrv].images.map((img) => (
                            <div key={img} className="relative h-full w-full overflow-hidden rounded-2xl border border-slate-100 bg-white">
                              <Image src={img} alt="service" fill className="object-contain" priority />
                            </div>
                          ))}
                        </div>
                      ) : (
                        <Image
                          src={services[activeSrv].images[0]}
                          alt="service"
                          fill
      ...
      {/* SECTION 2 : ESPACE DOCUMENTAIRE - Hauteur réduite pour plus de punch */}
        ...
        <div className="sticky top-0 min-h-screen flex flex-col justify-start md:justify-center bg-slate-50 overflow-hidden border-t border-slate-200 pt-24 md:pt-0 text-slate-900">
          <div className="max-w-7xl mx-auto w-full px-6">
            ...
            <div className="relative min-h-[450px] md:h-[500px] flex items-center justify-center">
              <AnimatePresence mode="wait">
                <motion.div key={activeDoc} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }} className="flex flex-col lg:flex-row items-center gap-12 md:gap-24 w-full">
                      ...
                      {documents[activeDoc].images.map((img, idx) => {
                        ...
                        return (
                          <motion.div key={img} className="absolute inset-0 rounded-2xl overflow-hidden shadow-2xl border-2 border-white bg-white" style={{ zIndex: 10 - idx }}
                            variants={{
                              initial: { x: isBack ? -30 : 0, y: isBack ? -10 : 0, scale: isBack ? 0.95 : 1, rotate: isBack ? -2 : 0, opacity: 1 },
                              animate: { x: isBack ? (isMobile ? -90 : -200) : (isMobile ? -8 : -12), y: isBack ? -15 : 0, scale: 1, rotate: isBack ? -4 : -1, transition: { duration: 0.4 } }
                            }}
                          >
                  ...
                  <div className="w-full lg:w-1/2 text-center lg:text-left space-y-4 md:space-y-6 lg:pl-12">
                    ...
                    {documents[activeDoc].downloadable && (
                      <a
                        href={documents[activeDoc].images[0]}
                        download
                        className="inline-flex items-center justify-center rounded-full border border-primary px-5 py-2 text-sm font-semibold text-primary transition-colors hover:bg-primary hover:text-white"
                      >
                    ...
                    <div className="flex justify-center lg:justify-start gap-2 pt-4">
                      {documents.map((_, i) => (
                        <button key={i} onClick={() => scrollToIndex(docsRef, i, documents.length)} className={`h-2 rounded-full transition-all duration-300 ${activeDoc === i ? "w-8 md:w-12 bg-slate-400" : "w-2 bg-slate-200 hover:bg-slate-300"}`} />
                      ))}
                    </div>
                  </div>
                </motion.div>
